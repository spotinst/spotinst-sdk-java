...
public class AzureStatefulNodeExample {
    ...
    private static String createStatefulNode(SpotinstAzureStatefulNodeClient client) {
        ...
        StatefulNodeVmSizesConfiguration vmSizes = vmSizesBuilder.setOdSizes(odSizes).setSpotSizes(spotSizes).setPreferredSpotSizes(preferredSpotSizes).build();

        //Build MarketPlace
        LaunchSpecMarketplaceConfiguration.Builder marketPlaceBuilder = LaunchSpecMarketplaceConfiguration.Builder.get();
        LaunchSpecMarketplaceConfiguration marketplace = marketPlaceBuilder.setPublisher("Canonical").setOffer("UbuntuServer").setSku("18.04-LTS").setVersion("latest").build();
        ...
        LaunchSpecNetworkSecurityGroupConfiguration.Builder networkSecurityGroupBuilder =  LaunchSpecNetworkSecurityGroupConfiguration.Builder.get();
        LaunchSpecNetworkSecurityGroupConfiguration networkSecurityGroup = networkSecurityGroupBuilder.setName("ManualQA-NSG-Public")
                                                .setResourceGroupName("ManualQAResourceGroup").build();
        ...
        LaunchSpecOsDiskSpecification osDisk = osDiskBuilder.setName("OsTestDisk").setType("Standard_LRS").setSizeGB(33).build();

        //Build Extension Protected Settings
        LaunchSpecExtensionsProtectedSettings protectedSettings = LaunchSpecExtensionsProtectedSettings.Builder.get()
                                                .setScript("IyEvYmluL2Jhc2gKZWNobyAibmlyIiA+IC9ob21lL25pci9uaXIudHh0Cg==").build();

        //Build Extentions
        LaunchSpecExtensionsSpecification.Builder extensionBuilder = LaunchSpecExtensionsSpecification.Builder.get();
        LaunchSpecExtensionsSpecification extensions = extensionBuilder.setName("extensionName").setType("customScript").setPublisher("Microsoft.Azure.Extensions")
                                          .setApiVersion("2.0").setMinorVersionAutoUpgrade(true).setProtectedSettings(protectedSettings).build();

        List<LaunchSpecExtensionsSpecification> extensionsList = Collections.singletonList(extensions);
        // Build Login
        LaunchSpecLoginSpecification login = LaunchSpecLoginSpecification.Builder.get().setUserName("ubuntu").setPassword("NetApp@123@321").build();

        // Build Tags
        LaunchSpecTagsSpecification tags = LaunchSpecTagsSpecification.Builder.get().setTagKey("creator").setTagValue("spotau@netapp.com").build();
        ...
        StatefulNodeLaunchSpecification launchSpecification = launchSpecificationBuilder.setImage(image).setNetwork(network).setDataDisks(dataDisks).setOsDisk(osDisk)
        ...
        StatefulNodeSignalConfiguration signal =  StatefulNodeSignalConfiguration.Builder.get().setEnabledAt(1624).setTimeout(180)
                                                  .setType(ElastigroupVmSignalEnumAzure.vmReady).build();
        List<StatefulNodeSignalConfiguration> signalList = Collections.singletonList(signal);

        //Build RevertToSpot
        StatefulNodeRevertToSpotConfiguration revertToSpot = StatefulNodeRevertToSpotConfiguration.Builder.get().setPerformAt(AzurePerformAtEnum.ALWAYS).build();
        //Build Strategy
        StatefulNodeStrategyConfiguration.Builder strategyBuilder = StatefulNodeStrategyConfiguration.Builder.get();
        StatefulNodeStrategyConfiguration strategy = strategyBuilder.setSignals(signalList).setFallbackToOd(true).setDrainingTimeout(180).setOrientation("AVAILABILITY")
                                                     .setPreferredLifecycle(AzureLifeCycleTypeEnum.SPOT).setRevertToSpot(revertToSpot).build();

        //Build Scheduling Tasks
        StatefulNodeTasksConfiguration.Builder tasksBuilder1 = StatefulNodeTasksConfiguration.Builder.get();
        StatefulNodeTasksConfiguration task1 = tasksBuilder1.setIsEnabled(true).setCronExpression("0 1 * * *").setType("pause")
                                                          .setAdjustment(1).setIdentifier(-1636758474).build();

        StatefulNodeTasksConfiguration.Builder tasksBuilder2 = StatefulNodeTasksConfiguration.Builder.get();
        StatefulNodeTasksConfiguration task2 = tasksBuilder2.setIsEnabled(false).setCronExpression("37 * * * *").setType("resume")
                                                            .setIdentifier(2085552106).build();
        List<StatefulNodeTasksConfiguration> tasksList = new ArrayList<>();
        tasksList.add(task1);
        tasksList.add(task2);

        //Build Scheduling
        StatefulNodeSchedulingConfiguration scheduling = StatefulNodeSchedulingConfiguration.Builder.get().setTasks(tasksList).build();
        ...
        StatefulNodeHealthConfiguration health = healthBuilder.setHealthCheckTypes(healthCheckTypes).setUnhealthyDuration(300).setGracePeriod(180).setAutoHealing("true").build();
        ...
        StatefulNode statefulNode = statefulNodeBuilder.setName("Automation-java-SDK-StatefulNode-Bansi").setRegion("eastus").setResourceGroupName("ManualQAResourceGroup")
                                                       .setDescription("stateful node for tests").setCompute(compute).setStrategy(strategy).setScheduling(scheduling)
                                                       .setPersistent(persistent).setHealth(health).build();
